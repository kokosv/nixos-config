{ lib, ... }: {
  options.homeManager.zsh = lib.mkOption { type = lib.types.deferredModule; };

  config.homeManager.zsh = { pkgs, config, ... }:
    let
      zshAliases = import ./_zsh/config/aliases.nix;
      zshFunctions = import ./_zsh/config/functions.nix;
      zshPlugins = import ./_zsh/config/plugins.nix { inherit pkgs; };
    in
    {
      programs.zsh = {
        enable = true;
        ...
        syntaxHighlighting.enable = false; # older and conflicts with plugin zsh-syntax-highlighting
